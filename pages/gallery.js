...
export async function getStaticProps() {
  ...
  const [pageRes, socialMedia] = await Promise.all([
    contentful.getEntries({ content_type: 'galleryPage' }),
    fetchSocialMedia(),
  ])
...
const Gallery = ({ pageTitle, videos, images, socialMedia }) => {
  ...
  return (
    <Layout pageTitle={pageTitle} socialMedia={socialMedia}>
      <div className="-mt-[85px] pt-[85px] min-h-screen">
        <div className="py-3 lg:py-16 flex flex-col gap-3 lg:gap-6">
          <div className="container flex justify-center items-center flex-wrap">
            <div
              className={`container grid grid-flow-row ${videos.length > 1 && 'lg:grid-cols-2'} gap-4 px-2`}
            >
          ...
          <div className="w-full grid grid-cols-2 md:grid-cols-3 gap-2 container px-2">
            {images.map((image, index) => (
              <ImageLayout
                ...
                index={index}
                image={
                  'https:' + image.url + '?w=500&h=500&fit=fill&fm=webp&q=80'
                }
      ...
      {selectedImage !== null && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur p-4 pt-20"
          onClick={closeModal}
        >
          <div
